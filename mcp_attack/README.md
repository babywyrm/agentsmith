...
After install, `mcp-attack` and `mcp-audit` are available as CLI commands:
...
# Targets from file (one URL per line, # comments ignored)
mcp-attack --targets-file urls.txt

# Built-in public targets (DVMCP localhost)
mcp-attack --public-targets
...
# Debug output
mcp-attack --targets http://localhost:2266 --debug
```

All commands also work with `mcp-audit`, `python3 -m mcp_attack`, or `python3 mcp_attack/mcp_audit.py`.
...
```bash
mcp-attack --port-range localhost:9001-9010 --json dvmcp_report.json
```
...
3. Run scanner: `mcp-attack --port-range localhost:9001-9010 --verbose`
